...
import { getProviderCredentials, clearRecoveredProviderState, markAccountUnavailable } from "@/sse/services/auth";
...
export async function createEmbeddingResponse(
  ...
  // 401 revoked, 403 forbidden, 404 model gone, 5xx server errors) so the
  // next embedding request skips this account. Mirrors chat.ts behavior.
  ...
  if (
    ...
    markAccountUnavailable(
      ...
      log.debug("EMBED", `Cooldown write failed for ${provider}/${credentials.connectionId?.slice(0, 8)}: ${err}`);
    });
  }
